    ...
    <section>
        <div class="mb-6 py-4 bg-white rounded">
            <div class="flex px-6 pb-4 border-b">
                ...
                <div class="ml-auto">
                    <a href="/admin/users_topi/create"
                ...
                <table class="table-auto w-full">
                    <thead>
                        <tr class="text-xs text-gray-500 text-left">
                            <th class="font-medium">列席者</th>
                            <th class="font-medium">15.5文字見出し</th>
                            <th class="font-medium">トピ写真（正方形）</th>
                    ...
                    <tbody>
                        ...
                        @foreach ($topis as $topi)
                            @if ($topi->administrators_id = Auth::guard('administrators')->user()->id)
                                <tr @class(['text-sm', 'bg-gray-50' => $loop->odd])>
                                    <td class="font-medium">
                                        <p class="font-medium"><a
                                                href="{{ route('admin.users_topi.edit', ['topi' => $topi->id]) }}">{{ $topi->name }}</a>
                                        </p>
                                    </td>
                                    <td class="font-medium">{{ $topi->title }}</td>
                                    <td class="flex px-4 py-3 items-center"><img
                                            class="w-12 h-12 mr-4 object-cover rounded-md"
                                            src="{{ asset('storage/' . $topi->hl_image) }}" alt=""></td>
                                    <td class="font-medium"></td>
                                    <td>{{ $topi->updated_at }}</td>
                                    <td>
                                        <div class="flex">
                                            <a class="mr-2"
                                                href="{{ route('admin.users_topi.edit', ['topi' => $topi->id]) }}">
                                            ...
                                            <form action="{{ route('admin.users_topi.destroy', ['topi' => $topi->id]) }}"
                                                method="POST">
                                                @csrf
                                                @method('DELETE')
                                                <button>
